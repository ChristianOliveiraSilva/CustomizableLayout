...
function changeJSON(res, newObj, force) {
    fs.readFile(filePath, 'utf8', function(err, data) {
        if (err) throw err
        let obj = JSON.parse(data)
        let merged = mergeRecursive(obj, newObj, force)
...
function mergeRecursive(obj1, obj2, force) {
    for (var p in obj2) {
        if (obj2[p] == null) {
            continue
        }
        ...
        try {
            if ( obj2[p].constructor == Object || (force && Array.isArray(obj2[p])) ) {
                obj1[p] = mergeRecursive(obj1[p], obj2[p], force)
            } else {
                obj1[p] = obj2[p]
            }
        } catch(e) {
